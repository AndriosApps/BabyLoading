...
public class SegmentedControlButton extends RadioButton {
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (isChecked()) {
            GradientDrawable grad = new GradientDrawable(Orientation.TOP_BOTTOM, new int[] { 0xfffff000, 0xffff3399 });
            grad.setBounds(0, 0, this.getWidth(), this.getHeight());
            grad.draw(canvas);
            textPaint.setColor(Color.WHITE);
        } else {
            GradientDrawable grad = new GradientDrawable(Orientation.TOP_BOTTOM, new int[] { 0xffa5a5a5, 0xffff6699 });
            grad.setBounds(0, 0, this.getWidth(), this.getHeight());
            grad.draw(canvas);
